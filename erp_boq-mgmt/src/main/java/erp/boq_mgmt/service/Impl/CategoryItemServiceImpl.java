...
public class CategoryItemServiceImpl implements CategoryItemService {
	...
	public CustomResponse getTypewiseCategoryItems(SearchDTO search) {
		...
		try {
			...
			if (search.getTypeId().intValue() == WorkorderTypes.Highway.getTypeId()) {
				List<HighwayBoqMapping> hbms = hbmDao.fetchCategoriesBoqs(search);
				if (hbms != null) {
					for (HighwayBoqMapping hbm : hbms) {
						if (hbm.getCategory() != null)
							keepCategoriesId.add(hbm.getCategory().getId());
					}
				}
