...
import erp.boq_mgmt.dto.HighwayBoqQtyImportDTO;
...
@Transactional
@Service
public class HighwayBoqMapServiceImpl implements HighwayBoqMapService {
	...
	@Override
	public CustomResponse mapCategoryBoq(HighwayBoqMappingDTO bcmDTO) {

		try {
			HighwayBoqMapping bcm = setObject.boqCategoryMapDtoToEntity(bcmDTO);
			bcm.setVersion(0);
			bcm.setQuantityVersion(0);
			Long id = bcmDao.mapCategoryBoq(bcm);
			if (id != null && id.longValue() > 0L) {
				HighwayBoqTransacs bct = setObject.highwayBoqEntityMappingToTransac(bcm);
				bcmDao.saveBoqCategoryTransac(bct);
			}
	...
	public CustomResponse getCategoryBoqById(SearchDTO search) {
		...
		try {
			HighwayBoqMapping bcm = bcmDao.fetchCategoryBoqById(search.getCatBoqId());
			HighwayBoqMappingDTO result = setObject.boqCategoryMapEntityToDto(bcm);
	...
	public CustomResponse getCategoriesBoqs(SearchDTO search) {
		...
		try {
			...
			List<HighwayBoqMappingDTO> result = new ArrayList<HighwayBoqMappingDTO>();
			if (bcmList != null) {
	...
	@Override
	public CustomResponse getHighwayBoqs(SearchDTO search) {

		try {
			List<HighwayBoqMapping> bcmList = bcmDao.fetchCategoriesBoqs(search);
			List<HighwayBoqMappingDTO> bcmListDTO = new ArrayList<HighwayBoqMappingDTO>();
			if (bcmList != null) {
				for (HighwayBoqMapping bcm : bcmList) {
			...
			if (bcmListDTO != null && bcmListDTO.size() > 0) {
				...
				HighwayBoqsRenderDTO result = setObject.renderChildParentHighwayBoqs(bcmListDTO, categoriesDTO);
				return new CustomResponse(Responses.SUCCESS.getCode(), result, Responses.SUCCESS.toString());
			}
	...
	@Override
	public CustomResponse updateCategoryBoq(HighwayBoqMappingDTO bcmDTO) {

		try {
			HighwayBoqMapping dbObj = bcmDao.fetchCategoryBoqById(bcmDTO.getId());
			if (dbObj == null)
			...
			if (!dbObj.getQuantity().equals(bcmDTO.getQuantity()))
				qtyDiff = true;
			HighwayBoqMapping bcmObj = setObject.updatedBoqCategoryMapping(dbObj,
			...
			Boolean isSaved = bcmDao.updateCategoryBoqMapping(bcmObj);
			if (needTransac) {
				HighwayBoqTransacs bcmTransac = setObject.highwayBoqEntityMappingToTransac(bcmObj);
				bcmTransac.setCreatedOn(new Date());
				bcmDao.saveBoqCategoryTransac(bcmTransac);
			}
	...
	@Override
	public CustomResponse importHighwayBoqQuantityMappingExcel(MultipartFile excelFile, SearchDTO search) {
		...
		try (XSSFWorkbook workbook = new XSSFWorkbook(excelFile.getInputStream())) {
			...
			List<HighwayBoqQtyImportDTO> hbqObjList = new ArrayList<>();
								...
								for (int m = initialRowIndex + 1; m < worksheet.getPhysicalNumberOfRows() + 1; m++) {
									...
									if (m == worksheet.getPhysicalNumberOfRows()) {
										if (hbqObjList != null) {
											...
											for (HighwayBoqQtyImportDTO obj : hbqObjList) {
											...
											List<HighwayBoqMapping> toSaveObjList = new ArrayList<>();
											int hbqObjListSize = hbqObjList.size();
											...
											for (int d = 0; d < hbqObjListSize; d++) {
												search.setCategoryId(null);
												search.setSubcategoryId(null);
												HighwayBoqQtyImportDTO excelObj = hbqObjList.get(d);
												...
												if ((excelObj.getStandardBookCode() != null
													...
													if (excelObj.getUnit() != null
														...
														HighwayBoqMapping obj = new HighwayBoqMapping(null, null, null,
																null, null, null, null, null, null, null,
																search.getSiteId(), uploadedFile, null, null, true,
																new Date(), search.getUserId(), search.getCompanyId());
														obj.setQuantity(
													...
													} else if (d < hbqObjListSize - 1) {
														int e = d;
														HighwayBoqQtyImportDTO excelCatObj = hbqObjList.get(e);
														if ((excelCatObj.getStandardBookCode() != null
															...
															for (int f = e + 1; f < hbqObjListSize; f++) {
																search.setCategoryId(categoryId);
																search.setSubcategoryId(null);
																HighwayBoqQtyImportDTO excelNestedObj = hbqObjList
																		.get(f);
																...
																if (excelNestedObj.getCode() != null && excelNestedObj
																	...
																	search.setCategoryId(categoryId);
																	search.setSubcategoryId(null);
																	HighwayBoqMapping obj = new HighwayBoqMapping(null,
																			null, null, null,
																			new CategoryItem(categoryId), null, null,
																			null, null, null, search.getSiteId(),
																			uploadedFile, null, null, true, new Date(),
																	...
																	if (excelNestedObj.getUnit() != null
																		...
																		for (int g = f + 1; g < hbqObjListSize; g++) {
																			HighwayBoqQtyImportDTO excelNestedBoqObj = hbqObjList
																					.get(g);
																			search.setCategoryId(categoryId);
																			...
																			HighwayBoqMapping nestedObj = new HighwayBoqMapping();
											...
											if (toSaveObjList != null) {
												...
												List<HighwayBoqMapping> hbqs = bcmDao.fetchCategoriesBoqs(search);
												List<ChainageBoqQuantityMapping> cbqs = cbqDao.fetchBoqWiseCbq(search);
												List<WorkorderBoqWorkQtyMapping> woBoqQtys = woBoqWorkDao
														.fetchWoBoqWorkQtys(search);
												for (HighwayBoqMapping hbqObj : toSaveObjList) {
													if (hbqObj.getBoq() == null)
														continue;

													List<ChainageBoqQuantityMapping> hbmCbqs = new ArrayList<ChainageBoqQuantityMapping>();
													if (cbqs != null) {
														for (ChainageBoqQuantityMapping obj : cbqs) {
															if (obj.getBoq().getId().equals(hbqObj.getBoq().getId())) {
																hbmCbqs.add(obj);
															}
														}
													}
													...
													for (int cbqIdx = 0; cbqIdx < hbmCbqs.size(); cbqIdx++) {
														ChainageBoqQuantityMapping toSaveObj = hbmCbqs.get(cbqIdx);
														if (cbqIdx == 0) {
															totalArea += (toSaveObj.getLhsQuantity()
																	+ toSaveObj.getRhsQuantity());
															continue;
														}
														ChainageBoqQuantityMapping previousToSaveObj = hbmCbqs
																.get(cbqIdx - 1);
													...
													if (hbqs != null) {
														for (HighwayBoqMapping hbq : hbqs) {
															if (hbq.getBoq().getId().equals(hbqObj.getBoq().getId())) {
																...
																if (!(hbq.getQuantity().equals(hbqObj.getQuantity())
																	...
																	hbq.setRate(
																			hbqObj.getRate() != null ? hbqObj.getRate()
																					: 0.0);
																	...
																	bcmDao.forceUpdateCategoryBoq(hbq);
													...
													if (!isHbqSaved) {
														...
														HighwayBoqMapping hbm = bcmDao.forceSaveCategoryBoq(hbqObj);
														if (hbm != null) {
												...
												if (hbqs != null) {
													for (HighwayBoqMapping hbq : hbqs) {
														...
														for (HighwayBoqMapping obj : hbqToReturn) {
														...
														if (toRemove) {
															...
															bcmDao.forceUpdateAfterDetachCategoryBoq(hbq);
															HighwayBoqTransacs hbqTransac = setObject
																	.highwayBoqEntityMappingToTransac(hbq);
															hbqTransac.setCreatedOn(new Date());
															bcmDao.saveBoqCategoryTransac(hbqTransac);
										...
										HighwayBoqQtyImportDTO hbqImport = new HighwayBoqQtyImportDTO(code, sdbCode,
												description, vendorDescription, unit, rate, maxRate, quantity, remark,
												m);
										hbqObjList.add(hbqImport);
									}
								}
							}
	...
	@Override
	public CustomResponse deactivateHighwayBoqMapping(SearchDTO search) {

		try {
			HighwayBoqMapping hbq = bcmDao.fetchCategoryBoqById(search.getHighwayBoqId());
			search.setHighwayBoqId(hbq.getId());
			...
			if (inWorkorder) {
				return new CustomResponse(Responses.FORBIDDEN.getCode(), false,
						"This highway BOQ is used in workorder.");
			}
			List<ChainageBoqQuantityMapping> cbqs = cbqDao.fetchBoqWiseCbq(search);
			if (cbqs != null) {
				for (ChainageBoqQuantityMapping obj : cbqs) {
					if (obj.getBoq().getId().equals(hbq.getBoq().getId())) {
						...
						obj.setModifiedBy(search.getUserId());
						obj.setModifiedOn(new Date());
						Boolean cbqUpdate = cbqDao.updateChainageBoqQuantityMapping(obj);
						if (cbqUpdate) {
							ChainageBoqQuantityTransacs cbqTransac = setObject.cbqEntityMappingToTransac(obj);
							cbqTransac.setCreatedOn(new Date());
			...
			hbq.setIsActive(false);
			bcmDao.forceUpdateAfterDetachCategoryBoq(hbq);
			HighwayBoqTransacs hbqTransac = setObject.highwayBoqEntityMappingToTransac(hbq);
			hbqTransac.setCreatedOn(new Date());
			bcmDao.saveBoqCategoryTransac(hbqTransac);
