...
//					"Provide valid item rate for item at index " + index);
				...
				if (workItem.getRunningMode() != null
						&& workItem.getRunningMode().equals(MachineryRunningMode.SHIFTS)) {
					if (workItem.getScheduleId() == null || workItem.getScheduleId() < 0)
						return new CustomResponse(Responses.BAD_REQUEST.getCode(), null,
								"Provide valid shift hours for item at index " + index);

				} else {
					workItem.setScheduleId(null);
				}

				index++;
			}

			Map<Long, Set<Integer>> machineCategoryScheduleMap = hmWork.getHmWorkItems().stream()
					.filter(o -> o.getRunningMode() != null && o.getRunningMode().equals(MachineryRunningMode.SHIFTS))
					.collect(Collectors.groupingBy(WorkorderHiringMachineWorkItemAddUpdateRequest::getMachineCategoryId,
							Collectors.mapping(WorkorderHiringMachineWorkItemAddUpdateRequest::getScheduleId,
									Collectors.toSet())));
			if (machineCategoryScheduleMap != null)
				for (Map.Entry<Long, Set<Integer>> entry : machineCategoryScheduleMap.entrySet()) {
					if (entry.getValue().size() > 1)
						return new CustomResponse(Responses.BAD_REQUEST.getCode(), null,
								"Same Category can't contain different shift hours");
				}
		}
...
//			return new CustomResponse(Responses.BAD_REQUEST.getCode(), null, "Provide valid item rate.");
		...
		if (workItem.getRunningMode() != null && workItem.getRunningMode().equals(MachineryRunningMode.SHIFTS)) {
			if (workItem.getScheduleId() == null || workItem.getScheduleId() < 0)
				return new CustomResponse(Responses.BAD_REQUEST.getCode(), null, "Provide valid shift hours for item.");
...
//			}
		...
		if (requestDTO.getAttendanceStatus() == null) {
			return new CustomResponse(Responses.BAD_REQUEST.getCode(), null, "Provide valid Attendance Status.");
		}

		if (requestDTO.getRunningMode().equals(MachineryRunningMode.SHIFTS)) {
			if (requestDTO.getBreakdownHours() != null
					&& (requestDTO.getBreakdownHours() < 0 || requestDTO.getBreakdownHours() > 24))
