...
import erp.billing.util.SwaggerApiOperationUtil;
import io.swagger.annotations.ApiOperation;

@RequestMapping(value = "/bill_type")
@RestController
@CrossOrigin
public class BillTypeController {
	...
	@ApiOperation(value = "Get bill types", notes = SwaggerApiOperationUtil.getBillTypesAPIReqParams)
	@RequestMapping(value = "/bill_types", method = RequestMethod.POST)
	public ResponseEntity<?> getBillTypes(@RequestBody(required = true) SearchDTO search, HttpServletRequest request)
			throws Exception {
		...
		if (response.getStatus().equals(Responses.SUCCESS.getCode()))
			response = billTypeService.getBillTypes(search);
