...
@RequestMapping(value = "/wo_labour_type")
@RestController
@CrossOrigin
public class WorkorderLabourTypeController {

	private final Logger LOGGER = LoggerFactory.getLogger(this.getClass());

	@Autowired
	private WorkorderLabourTypeService workorderLabourTypeService;

	@Autowired
	private AuthorityService roleAuthorityUtil;

	@ApiOperation(value = "Add Or Update Workorder Labour Type")
	@RequestMapping(value = "v1/add_or_update", method = RequestMethod.POST)
	public ResponseEntity<?> addOrUpdateWorkorderLabourType(
			@RequestBody(required = true) WorkorderLabourTypeAddUpdateRequest requestObj, HttpServletRequest request)
			throws Exception {
		...
		if (response.getStatus().equals(Responses.SUCCESS.getCode()))
			response = workorderLabourTypeService.addOrUpdateWorkorderLabourType(requestObj);
	...
	@ApiOperation(value = "Get Workorder Labour Type List")
	@RequestMapping(value = "v1/get/all/{companyId}", method = RequestMethod.GET)
	public ResponseEntity<?> getWorkorderLabourTypeList(
		...
		if (response.getStatus().equals(Responses.SUCCESS.getCode()))
			response = workorderLabourTypeService.getWorkorderLabourTypeList(companyId);
	...
	@ApiOperation(value = "Get Workflow Labour Type By Id")
	@RequestMapping(value = "v1/get/{id}", method = RequestMethod.GET)
	public ResponseEntity<?> getWorkflowLabourTypeById(@PathVariable(value = "id", required = true) Integer id,
		...
		if (response.getStatus().equals(Responses.SUCCESS.getCode()))
			response = workorderLabourTypeService.getWorkorderLabourTypeById(id);
	...
	@ApiOperation(value = "Deactivate Workflow Labour Type")
	@RequestMapping(value = "v1/deactivate/{id}", method = RequestMethod.PUT)
	public ResponseEntity<?> deactivateWorkorderLabourTypeById(@PathVariable(value = "id", required = true) Integer id,
			HttpServletRequest request) throws Exception {
		...
		if (response.getStatus().equals(Responses.SUCCESS.getCode()))
			response = workorderLabourTypeService.deactivateWorkorderLabourType(id, user.getId());
